...
public class FriendManager extends BaseBusinessLogicManager implements
	...
	protected void onSuccessResponse(BaseResponse response, Handler handler) {
		if (response instanceof GetUserInfoResponse) {
			...
			handleresponse(Constans.GET_VERBOSEUSERINFO_SUCCESS,
					((GetVerboseUserInfoResponse) response)
							.getmGetVerboseInfoRsp(), handler);
		} else if (response instanceof AddFriendMsgResponse) {
			handleresponse(((AddFriendMsgResponse) response).getSendMsgRes()
					.getUint32Result() == 0 ? Constans.ADD_FRIEND_SUCCESS
					: Constans.ADD_FRIEND_FAILED,
					((AddFriendMsgResponse) response).getSendMsgRes(), handler);
	...
	public void addFriend(long systemId, String verifyMsg, Handler handler) {
		...
		// 发验证消息
		// AddFriendMsgRequest request = new
		// AddFriendMsgRequest(LogicManager.accountManager().getCurrentAccoutSystemId(),LogicManager.accountManager().getCurrentSessionId(),targetList);
		// VerifyRequest.Builder verifyMsgInfobuilder =
		// VerifyRequest.newBuilder();
		// verifyMsgInfobuilder.setUint32VerifyType(0);
		// verifyMsgInfobuilder.setStrVerifyInfo(verifyMsg);
		// request.setVerifyRequest(verifyMsgInfobuilder.build(),
		// ByteString.EMPTY);
		// 加好友操作
	...
	public void updateFriendRemark(long systemId, String remark, Handler handler) {
		UpdateFriendRequest request = new UpdateFriendRequest(LogicManager
				.accountManager().getCurrentSessionId(), LogicManager
				.accountManager().getCurrentAccoutSystemId());
	...
	public void delFriend(List<Long> systemIdList, Handler handler) {
		DelFriendRequest request = new DelFriendRequest(LogicManager
				.accountManager().getCurrentSessionId(), LogicManager
				.accountManager().getCurrentAccoutSystemId());
